...
def add(text,lesson=0,kind='misc'):
    text=norm(text)
    if not text:return
    key=hashlib.sha1(text.encode()).hexdigest(); row=items.setdefault(key,{'hash':key,'text':text,'lessons':[],'kinds':[]})
    if lesson and lesson not in row['lessons']:row['lessons'].append(int(lesson))
    if kind not in row['kinds']:row['kinds'].append(kind)
for v in data.get('vocabulary',[]):
    L=int(v.get('lesson') or 0);add(v.get('tts_text') or v.get('japanese'),L,'vocab');add(v.get('spelling_text'),L,'spelling')
    ex=v.get('example') or {}; add(ex.get('jp') or ex.get('japanese'),L,'sentence')
...
rows=sorted(items.values(),key=lambda x:x['hash']); build=ROOT/'_build';build.mkdir(exist_ok=True)
