...
has_today=("TODAY&apos;SSTUDY" in compact_dashboard or "TODAY'SSTUDY" in compact_dashboard)
has_completion_logic=bool(re.search(r"currentPct\s*>=\s*80\s*&&\s*currentDue\s*===\s*0",dashboard))
has_visible_rule=(("LESSON GOAL" in dashboard or "LESSON COMPLETION RULE" in dashboard)
                  and ("80% vocabulary mastered" in dashboard or "80% target" in dashboard)
                  and ("0 due SRS card" in dashboard or "0 due" in dashboard))
check(has_today,"Today's Study panel present")
check(has_completion_logic,"Transparent lesson completion rule encoded")
check(has_visible_rule,"Completion rule visible to learner")
