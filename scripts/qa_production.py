...
public_styles=(ROOT/"styles/public.scss").read_text(encoding="utf-8")
studio_styles=(ROOT/"styles/studio.scss").read_text(encoding="utf-8")
...
public_landing=(ROOT/"components/PublicLanding.tsx").read_text(encoding="utf-8")
public_free=(ROOT/"components/PublicFreeLab.tsx").read_text(encoding="utf-8")
language_provider=(ROOT/"lib/language.tsx").read_text(encoding="utf-8")
jlpt_resources=(ROOT/"lib/jlptResources.ts").read_text(encoding="utf-8")
...
check("Play the example" in (ROOT/"components/Vocabulary.tsx").read_text(encoding="utf-8"),"Vocabulary example audio has bilingual accessible name")
...
check("@/styles/public.scss" in layout and "@/styles/studio.scss" in app and "./foundation/design-system" in public_styles and "./modules/listening" in studio_styles,"Public/studio semantic stylesheet split")
...
check("total:6" in mock and "total:15" in mock and "total:30" in mock and "total:67" in mock,"Quick 6 / Lesson 15 / Mini 30 / Full 67 modes")
check("counts:{vocabulary:21,'grammar-reading':22,listening:24}" in mock.replace(" ",""),"Full 21/22/24 practice blueprint")
...
check("initialLanguage='bn'" in (ROOT/"components/AppEntry.tsx").read_text(encoding="utf-8") and "useLanguage" in public_landing and "nihongo_vibes_language_v1" in language_provider,"Public experience defaults to Bangla with persistent English switch")
check("public-free" in public_landing and "PublicFreeLab" in public_landing,"Pre-login free learning hub mounted")
check("public_free_phrase" in public_free and "public_free_listening" in public_free and "FREE DIAGNOSTIC" in public_free,"Free phrase, listening and diagnostic practice")
check(jlpt_resources.count("kind:'full-mock'")>=3 and "Official JLPT" in jlpt_resources,"Verified official and external mock directory")
...
check("AnalyticsConsent" in layout and "consent==='accepted'" in consent and "gtag('config'" in consent and "saveConsent('declined')" in consent,"Analytics remains explicit opt-in")
