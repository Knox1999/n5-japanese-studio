...
const CORE=['./','./?view=dashboard','./data/meta.json?v=50','./manifest.webmanifest'];
...
self.addEventListener('activate',event=>{
  event.waitUntil(
    caches.keys().then(keys=>Promise.all(
      keys
        .filter(k=>(k.startsWith('n5-')||k.startsWith('nihongo-vibes-')) && ![CORE_CACHE,RUNTIME_CACHE,AUDIO_CACHE].includes(k))
