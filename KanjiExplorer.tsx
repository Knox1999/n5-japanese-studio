...
function StrokeOrder({kanji}:{kanji:string}){
 ...
 useEffect(()=>{let dead=false;setLoading(true);setAvailable(false);(async()=>{try{const idx:Record<string,string>=await fetch(`${BASE}/assets/strokes/index.json`).then(async r=>r.ok?((await r.json()) as Record<string,string>):({} as Record<string,string>));const fn=idx[kanji];if(!fn)return;const text=await fetch(`${BASE}/assets/strokes/${fn}`).then(r=>r.ok?r.text():'');if(!text)return;raw.current=text;if(!dead){setAvailable(true);setTimeout(draw,30)}}catch(e){trackError('resource',e)}finally{if(!dead)setLoading(false)}})();return()=>{dead=true}},[kanji]);
