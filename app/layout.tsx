...
export const metadata: Metadata = {
  ...
  twitter: {
    card:'summary',
    title:'The Nihongo Vibes — N5 Japanese Learning Studio',
    description:'A connected futuristic Japanese N5 learning workspace.',
  ...
  return (
    <html lang="bn">
      <body>
        ...
        {children}
        <Script id="nihongo-schema" type="application/ld+json" strategy="afterInteractive">{JSON.stringify(schema)}</Script>
        <Script src={`https://www.googletagmanager.com/gtag/js?id=${ga}`} strategy="afterInteractive" />
        <Script id="ga4" strategy="afterInteractive">{`window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments)}window.gtag=gtag;gtag('js',new Date());gtag('config','${ga}',{send_page_view:false,anonymize_ip:true});`}</Script>
      </body>
    </html>
  );
}
