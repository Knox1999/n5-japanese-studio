...
function StrokeOrder({kanji}:{kanji:string}){
  ...
  if(!available) return <div className="stroke-empty"><Sparkles/> Stroke-order SVG unavailable for this character.</div>;
...
export default function KanjiExplorer({data}:{data:LessonPayload}){
 const [tree,setTree]=useState<KLCTree|null>(null),[memory,setMemory]=useState<KLCMemory>({}),[query,setQuery]=useState(''),[selected,setSelected]=useState(1),[limit,setLimit]=useState(96),[depth,setDepth]=useState(3),[browserOpen,setBrowserOpen]=useState(true);
 ...
 if(!tree||!node)return <div className="kanji-loading"><Loader2 className="animate-spin"/><b>Loading KLC construction tree…</b></div>;
 ...
 return <div className="space-y-5 pb-8"><section className="study-header tone-kanji"><div><div className="section-kicker">KLC Kanji Construction Tree</div><h1>{tree.stats.nodes?.toLocaleString()||'2,300'} Kanji · {tree.stats.edges?.toLocaleString()||tree.edges.length.toLocaleString()} exact component edges</h1><p className="font-bn">আপনার KLC node + edge CSV অনুযায়ী visual recognition tree। Visual construction ≠ historical etymology.</p></div><TreePine className="header-big-icon"/></section>
 ...
 <section className="lesson-kanji-strip"><div><span className="section-kicker">Lesson {String(data.lesson).padStart(2,'0')} Kanji</span><b>{lessonNodes.length}/{lessonChars.length} mapped</b></div><div className="kanji-chip-scroll">{lessonNodes.map(n=><button key={n[0]} className={n[0]===selected?'active':''} onClick={()=>open(n[0])}><span>{n[1]}</span><small>{n[2]}</small></button>)}{missing.map(ch=><button key={ch} disabled><span>{ch}</span><small>Not in KLC 2300</small></button>)}</div></section>
 <button className="kanji-browser-toggle premium-btn premium-btn-secondary" onClick={()=>setBrowserOpen(v=>!v)}>{browserOpen?<ChevronDown/>:<ChevronRight/>} Search all 2,300</button>
 <div className="kanji-layout"><AnimatePresence initial={false}>{browserOpen&&<motion.aside initial={{opacity:0,x:-8}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-8}} className="kanji-browser"><label className="search-field"><Search size={17}/><input value={query} onChange={e=>{setQuery(e.target.value);setLimit(96)}} placeholder="Kanji / KLC no / meaning / reading…" aria-label="সব Kanji-তে খুঁজুন"/></label><div className="kanji-result-grid">{results.map(n=><button key={n[0]} className={n[0]===selected?'active':''} onClick={()=>open(n[0])} aria-label={`KLC ${n[0]} · ${n[1]} · ${n[2]}`}><span>{n[1]}</span><small>#{n[0]}</small></button>)}</div>{limit<nodes.length&&<button className="premium-btn premium-btn-secondary w-full" onClick={()=>setLimit(x=>x+96)}>Load more</button>}</motion.aside>}</AnimatePresence>
 <main className="kanji-detail"><motion.article key={node.no} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} className="kanji-identity"><div className="kanji-glyph">{node.kanji}</div><div><span className="section-kicker">KLC #{node.no} · {node.status||'Record'}</span><h2>{node.meaning}</h2>{mem[0]&&<p className="font-bn text-lg font-bold text-sakura">{mem[0]}</p>}<div className="reading-pills"><span>音 {node.onyomi||'—'}</span><span>訓 {node.kunyomi||'—'}</span><span>{node.verification||'—'}</span></div></div></motion.article>
 <section className="premium-panel"><div className="section-kicker">Exact one-level construction</div><h2 className="section-title">{node.oneLevel||node.kanji}</h2>{edges.length?<div className="component-card-grid">{edges.map((e,i)=><button key={`${e[1]}-${i}`} onClick={()=>e[5]&&open(e[5])} className={e[6]==='LATER_KLC_MNEMONIC_REF'?'later-ref':''}><span className="position">{e[1]}</span><strong className="font-jp">{e[2]}</strong><div><b>{e[3]}</b><small>{typeLabel(e[4])}</small><em>{relationLabel(e[6])}</em></div></button>)}</div>:<div className="atomic-note">Atomic / root record — immediate component edge নেই।</div>}{mem[1]&&<div className="memory-story font-bn"><b>মনে রাখার ১-লাইন গল্প</b><p>{mem[1]}</p><small>Visual memory aid — true etymology নয়</small></div>}</section>
 <section className="premium-panel recursive-tree-panel"><div className="tree-panel-head"><div><div className="section-kicker">Recursive KLC Construction</div><h2 className="section-title">True expandable component tree</h2><p className="section-subtitle">Explicit KLC→KLC links recursively expand; RAD/primitive components leaf node হিসেবে থাকে।</p></div><label>Depth<select value={depth} onChange={e=>setDepth(Number(e.target.value))}>{[1,2,3,4].map(x=><option key={x}>{x}</option>)}</select></label></div><div className="recursive-tree-scroll"><RecursiveNode no={selected} depth={0} maxDepth={depth} byNo={byNo} byParent={byParent} onOpen={open}/></div></section>
 <div className="klc-info-grid-v42"><section className="premium-panel"><div className="section-kicker">Leaf components</div><div className="klc-token-row-v42">{splitTokens(node.leaf).map(x=><span key={x}>{x}</span>)}</div><div className="section-kicker mt-4">Leaf RAD components</div><div className="klc-token-row-v42 rad">{splitTokens(node.leafRad).map(x=><span key={x}>{x}</span>)}</div>{node.note&&<p className="verification-note">{node.note}</p>}</section><section className="premium-panel"><div className="section-kicker">Builds into</div><h2 className="section-title">Explicit KLC child links</h2><div className="builds-grid">{builds.length?builds.map(n=><button key={n[0]} onClick={()=>open(n[0])}><span>{n[1]}</span><small>KLC {n[0]}</small><b>{n[2]}</b></button>):<p className="section-subtitle">No explicit child link found.</p>}</div></section></div>
