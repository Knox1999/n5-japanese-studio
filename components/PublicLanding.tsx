...
export default function PublicLanding({onLogin,onJoin}:Props){
  ...
  return <div className={`${styles.page} ${language==='bn'?styles.banglaPage:''}`}>
    <header className={styles.header}>
      <div className={styles.headerInner}>
        ...
        <nav className={styles.desktopNav} aria-label={text('পাবলিক নেভিগেশন','Public navigation')}>
          ...
          <button type="button" onClick={()=>goTo('public-features')}>{text('ফিচার','Features')}</button>
          <button type="button" onClick={()=>goTo('public-journey')}>{text('কীভাবে কাজ করে','How it works')}</button>
          <button type="button" onClick={()=>goTo('public-preview')}>{text('স্টাডি ফ্লো','Study flow')}</button>
        </nav>
      ...
      {menuOpen&&<div className={styles.mobileMenu}>
        <button type="button" onClick={()=>goTo('public-free')}>{text('ফ্রি শেখা','Free learning')}</button>
        <button type="button" onClick={()=>goTo('public-features')}>{text('সব ফিচার','Features')}</button>
        <button type="button" onClick={()=>goTo('public-journey')}>{text('কীভাবে কাজ করে','How it works')}</button>
        <button type="button" onClick={()=>goTo('public-preview')}>{text('স্টাডি ফ্লো','Study flow')}</button>
    ...
    <main id="main-content">
      <section className={styles.hero}>
        <div className={styles.heroInner}>
          <div className={styles.heroCopy}>
            <div className={styles.eyebrow}><Sparkles size={15}/>{text('বাংলায় বুঝুন · Japanese-এ এগিয়ে যান','ONE FOCUSED PATH TO JLPT N5')}</div>
            <h1>{text('লগইনের আগেই JLPT N5','Start learning JLPT N5')} <span>{text('শেখা শুরু করুন।','before you sign in.')}</span></h1>
            <p className={language==='bn'?'font-bn':''}>{text('ফ্রি starter lesson, Japanese listening, diagnostic quiz এবং mock-test directory ব্যবহার করে আগে নিজেই যাচাই করুন—এই studio আপনার জন্য কতটা কাজে দেয়।','Try the free starter lesson, Japanese listening, diagnostic quiz and mock-test directory first. Create an account only when the studio has earned your trust.')}</p>
            <div className={styles.heroActions}>
              ...
              <button className={styles.secondaryCta} type="button" onClick={()=>goTo('public-preview')}>{text('স্টুডিও দেখুন','Explore the studio')}<ChevronRight size={17}/></button>
            </div>
            <div className={styles.trustRow}>
              <span><Check size={15}/>{text('ফ্রি অংশে কোনো login লাগবে না','No login for free materials')}</span>
              <span><Check size={15}/>{text('বাংলা ও English—দুই ভাষা','Bangla and English')}</span>
            </div>
          </div>
      ...
      <section className={styles.featureSection} id="public-features">
        <div className={styles.sectionHeading}>
          <span>{text('একটি সংযুক্ত স্টুডিও','ONE CONNECTED STUDIO')}</span>
          <h2>{text('প্রতিদিনের N5 প্রস্তুতির সবকিছু এক জায়গায়','Everything needed for daily N5 study')}</h2>
          <p className={language==='bn'?'font-bn':''}>{text('প্রতিটি tool-এর আলাদা কাজ আছে, কিন্তু progress ও review একই learning journey-তে যুক্ত থাকে।','Each tool has a clear job, while progress and review stay connected across the learning journey.')}</p>
        </div>
        <div className={styles.featureGrid}>
          {features.map(({icon:Icon,label,bn,en})=><article className={styles.featureCard} key={label}>
            <span className={styles.featureIcon}><Icon size={22}/></span><h3>{label}</h3><p className={language==='bn'?'font-bn':''}>{language==='bn'?bn:en}</p><span className={styles.cardLink}>{text('Account-এ অন্তর্ভুক্ত','Included with account')}<Check size={15}/></span>
          </article>)}
        </div>
      </section>
      ...
      <section className={styles.journeySection} id="public-journey">
        <div className={styles.journeyInner}>
          <div className={styles.sectionHeadingLeft}><span>{text('গাইডেড স্টাডি','GUIDED STUDY')}</span><h2>{text('প্রতিদিন অনুসরণ করার সহজ একটি rhythm','A simple rhythm you can repeat every day')}</h2><p className={language==='bn'?'font-bn':''}>{text('বড় syllabus-কে ছোট, পরিষ্কার এবং কাজে লাগে এমন next step-এ ভাগ করা হয়েছে।','The app turns a large syllabus into a short sequence of useful next steps.')}</p></div>
      ...
      <section className={styles.previewSection} id="public-preview">
        <div className={styles.previewInner}>
          <div className={styles.previewCopy}>
            ...
            <h2>{text('কী আছে শুধু নয়—এরপর কী পড়বেন, সেটাও জানুন।','Know what to study next, not just what exists.')}</h2>
            <p className={language==='bn'?'font-bn':''}>{text('Account workspace lesson, progress, review ও mistakes একসাথে রাখে—তাই পরের দরকারি কাজটি সবসময় পরিষ্কার থাকে।','Your account keeps lessons, progress, review and mistakes together so the next useful action stays obvious.')}</p>
            <ul>
      ...
      <section className={styles.featureSection} id="public-trust">
        <div className={styles.sectionHeading}><span>{text('স্বচ্ছতা ও বিশ্বাস','TRUST THROUGH TRANSPARENCY')}</span><h2>{text('কী শিখবেন, data কোথায় থাকে এবং score কী বোঝায়—সব পরিষ্কার।','Clear learning scope, data controls and honest scoring.')}</h2><p className={language==='bn'?'font-bn':''}>{text('এটি একটি independent Bangla-first self-study studio; official JLPT affiliation দাবি করে না এবং fabricated testimonial ব্যবহার করে না।','This is an independent Bangla-first self-study studio. It does not claim official JLPT affiliation or use fabricated testimonials.')}</p></div>
        <div className={styles.featureGrid}>
          <article className={styles.featureCard}><span className={styles.featureIcon}><BookOpen size={22}/></span><h3>{text('২৫টি guided lesson','25 guided lessons')}</h3><p>{text('শব্দ → grammar → listening → recall → test—পরিষ্কার daily flow।','A clear words → grammar → listening → recall → test flow.')}</p><span className={styles.cardLink}>{text('Lesson-based curriculum','Lesson-based curriculum')}<Check size={15}/></span></article>
          <article className={styles.featureCard}><span className={styles.featureIcon}><ShieldCheck size={22}/></span><h3>{text('আপনার data, আপনার নিয়ন্ত্রণ','Your data, your control')}</h3><p>{text('Cloud sync-এর পাশাপাশি JSON export ও permanent account deletion।','Cloud sync with JSON export and permanent account deletion.')}</p><a className={styles.cardLink} href={`${basePath}/privacy/`}>{text('Privacy পড়ুন','Read privacy')}<ChevronRight size={15}/></a></article>
          <article className={styles.featureCard}><span className={styles.featureIcon}><Target size={22}/></span><h3>{text('সৎ mock scoring','Honest mock scoring')}</h3><p>{text('Practice percentage-কে official JLPT scaled score বলা হয় না; প্রতিটি ভুল review করা যায়।','Practice percentages are never presented as official scaled scores; every mistake is reviewable.')}</p><a className={styles.cardLink} href="https://github.com/Knox1999/n5-japanese-studio/issues" target="_blank" rel="noreferrer noopener">{text('সমস্যা জানান','Report an issue')}<ChevronRight size={15}/></a></article>
        </div>
      </section>

      <section className={styles.finalCta}>
        <div><span>{text('যখন প্রস্তুত','READY WHEN YOU ARE')}</span><h2>{text('আপনার N5 study workspace খুলুন।','Open your N5 study workspace.')}</h2><p className={language==='bn'?'font-bn':''}>{text('ফ্রি materials ব্যবহার করার পর account খুলে পুরো learning system চালিয়ে যান।','After trying the free materials, create an account and continue with the full learning system.')}</p></div>
    ...
    <footer className={styles.footer}>
      ...
      <nav aria-label={text('সহায়তা ও নীতিমালা','Support and policies')}><a href={`${basePath}/privacy/`}>{text('গোপনীয়তা','Privacy')}</a><a href={`${basePath}/terms/`}>{text('ব্যবহারের শর্ত','Terms')}</a><a href="https://github.com/Knox1999/n5-japanese-studio/issues" target="_blank" rel="noreferrer noopener">{text('সহায়তা','Support')}</a></nav>
