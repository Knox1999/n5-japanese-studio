...
export default function Shell({meta,lesson,view,onLesson,onView,children}:{meta:StudioMeta;lesson:number;view:ViewName;onLesson:(n:number,v?:ViewName)=>void;onView:(v:ViewName)=>void;children:React.ReactNode}){
 ...
 useEffect(()=>{stopAudio();setDrawer(false);setSearch(false);setLessonPicker(false);const html=document.documentElement,body=document.body;html.classList.remove('overlay-open');body.classList.remove('overlay-open');for(const prop of ['overflow','position','height'] as const)body.style.removeProperty(prop);for(const prop of ['overflow','height'] as const)html.style.removeProperty(prop)},[view,lesson]);
 useEffect(()=>{const onWheel=(e:WheelEvent)=>{if(document.documentElement.classList.contains('overlay-open')||document.body.classList.contains('overlay-open'))return;if(Math.abs(e.deltaY)<=Math.abs(e.deltaX))return;const target=e.target instanceof Element?e.target:null;if(!target)return;const horizontal=target.closest('.future-subnav,.future-protocol,.grammar-matrix-scroll,.word-filter-row,.verb-mode-tabs');if(!horizontal)return;e.preventDefault();window.scrollBy({top:e.deltaY,left:0,behavior:'auto'})};document.addEventListener('wheel',onWheel,{passive:false});return()=>document.removeEventListener('wheel',onWheel)},[]);
