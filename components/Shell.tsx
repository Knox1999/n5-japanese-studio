...
export default function Shell({meta,lesson,view,onLesson,onView,children}:{meta:StudioMeta;lesson:number;view:ViewName;onLesson:(n:number,v?:ViewName)=>void;onView:(v:ViewName)=>void;children:React.ReactNode}){
 ...
 /* V51 scroll-rescue:
    Use a class instead of leaving inline body overflow behind.
    It also clears stale inline overflow from older releases. */
 useEffect(()=>{
 ...
 useLayoutEffect(()=>{const el=mainRef.current;if(!el||typeof window==='undefined'||window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;let ctx:any;(async()=>{try{const {gsap}=await import('gsap');ctx=gsap.context(()=>{
   ...
   gsap.fromTo(el,{opacity:.78,y:mobile?4:8,filter:mobile?'none':'blur(2px)'},{opacity:1,y:0,filter:'blur(0px)',duration:mobile?.26:.42,ease:'power3.out',clearProps:'filter'});
   gsap.fromTo('.future-subnav button.active',{scale:.96},{scale:1,duration:.28,ease:'back.out(1.5)'});
   const hero=el.querySelectorAll('.study-header,.future-page-hero');
   if(hero.length)gsap.fromTo(hero,{opacity:.72,y:mobile?4:9},{opacity:1,y:0,duration:mobile?.28:.46,ease:'power3.out'});
   ...
   if(cards.length)gsap.fromTo(cards,{opacity:.55,y:mobile?3:8},{opacity:1,y:0,duration:mobile?.22:.34,stagger:mobile?.01:.022,ease:'power2.out',clearProps:'transform'});
 ...
 const Brand=()=> <button className="future-brand future-brand-v49" onClick={()=>go('dashboard')} aria-label="Go to The Nihongo Vibes dashboard">
   <span className="future-brand-logo-wrap"><img className="future-brand-logo" src={`${process.env.NEXT_PUBLIC_BASE_PATH||''}/assets/nihongo-vibes-logo.webp`} alt="" /><i className="future-brand-pulse"/></span>
   <span><strong>THE NIHONGO VIBES</strong><small>日本語 · FUTURE LEARNING STUDIO</small></span>
 </button>;

 const LessonNode=()=> <div className="future-header-node" title={`Current lesson: ${lesson}`}>
   <Radio size={17}/><span>L{String(lesson).padStart(2,'0')}</span>
   <select value={lesson} onChange={e=>onLesson(Number(e.target.value),'dashboard')} aria-label="Current lesson">{meta.lessons.map(L=><option value={L.lesson} key={L.lesson}>Lesson {String(L.lesson).padStart(2,'0')} · {L.title}</option>)}</select><ChevronDown size={16}/>
 </div>;

 const Drawer=()=> <aside className="future-drawer" aria-label="Study navigation">
   <div className="future-drawer-head"><Brand/><button onClick={()=>setDrawer(false)} aria-label="Close menu"><X size={22}/></button></div>
   <div className="future-drawer-status"><i/><span>SYSTEM ONLINE</span><b>LESSON {String(lesson).padStart(2,'0')}</b></div>
   <div className="future-drawer-lesson"><label>CURRENT LESSON</label><div><select value={lesson} onChange={e=>{onLesson(Number(e.target.value),'dashboard');setDrawer(false)}}>{meta.lessons.map(L=><option value={L.lesson} key={L.lesson}>Lesson {String(L.lesson).padStart(2,'0')} · {L.title}</option>)}</select><ChevronDown size={18}/></div></div>
   <nav>{NAV.map(x=>{const I=x.icon;return <button key={x.view} className={view===x.view?'active':''} onClick={()=>go(x.view)} aria-current={view===x.view?'page':undefined}><I size={21}/><span>{x.label}</span>{x.view==='kanji'&&<small>2300</small>}</button>})}</nav>
   <div className="future-drawer-utilities">
     <button onClick={()=>window.dispatchEvent(new Event('n5-open-vault'))}><DatabaseBackup size={20}/><span>Backup & Restore</span></button>
     <button onClick={async()=>{try{await navigator.clipboard.writeText(location.href);track('share_link',{section_name:view,lesson_number:lesson})}catch{}}}><Share2 size={20}/><span>Copy current link</span></button>
   </div>
   <div className="future-drawer-foot"><Sparkles size={18}/><span>Learn → Listen → Recall → Use → Review</span></div>
 </aside>;

 return <div className="future-shell future-shell-v48 future-shell-v51" data-view={view}>
   ...
   <header className="future-header">
     ...
     <nav className="future-primary-nav" aria-label="Primary navigation">
       ...
       <button className={['kanji','mock','history'].includes(view)?'active':''} onClick={()=>setDrawer(true)}>More <ChevronDown size={15}/></button>
     </nav>
     <div className="future-header-tools">
       <button className="future-search-trigger" onClick={openSearch} aria-label="Search learning content"><Search size={20}/><span>Search</span><kbd>⌘K</kbd></button>
       <LessonNode/>
       <button className="future-menu-trigger" onClick={()=>setDrawer(true)} aria-label="Open menu"><Menu size={23}/></button>
     </div>
   </header>

   <div className="future-subnav" ref={subnavRef} role="navigation" aria-label="Quick access">
     <span><Command size={16}/> QUICK ACCESS</span>
     {NAV.slice(1).map(x=>{const I=x.icon;return <button key={x.view} data-view={x.view} className={view===x.view?'active':''} onClick={()=>go(x.view)} aria-current={view===x.view?'page':undefined}><I size={18}/>{x.short}</button>})}
   </div>
   ...
   <nav className="future-mobile-dock" aria-label="Mobile navigation">
     {([NAV[0],NAV[1],NAV[2],NAV[6]] as NavItem[]).map(x=>{const I=x.icon;return <button key={x.view} onClick={()=>go(x.view)} className={view===x.view?'active':''} aria-current={view===x.view?'page':undefined}><I size={24}/><span>{x.short}</span></button>})}
     <button onClick={()=>setDrawer(true)} aria-label="Open all navigation"><Menu size={24}/><span>Menu</span></button>
   </nav>
   ...
   {search&&<div className="future-search-layer" role="dialog" aria-modal="true" aria-label="Search learning content">
     ...
     <section className="future-search-dialog">
       <div className="future-search-head"><Search size={21}/><input autoFocus value={q} onChange={e=>setQ(e.target.value)} placeholder="Japanese / Kanji / বাংলা / English…" aria-label="Search"/><button onClick={()=>setSearch(false)} aria-label="Close search"><X size={22}/></button></div>
